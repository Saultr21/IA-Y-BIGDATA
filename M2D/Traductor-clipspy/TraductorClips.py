...
def extract_associations(root):
    ...
    for elem in root.findall('.//packagedElement'):
        if elem.get('{http://schema.omg.org/spec/XMI/2.1}type') != 'uml:Association':
            continue
        src, dst = elem.get('memberEnd').split()
        srcMult = dstMult = None
        whole   = part    = None
        for oe in elem.findall('ownedEnd'):
            mult = oe.get('multiplicity')
            if oe.get('type') == src:  srcMult = mult
            if oe.get('type') == dst:  dstMult = mult
            # ¿Es composición o agregación?
            agg = oe.get('aggregation')
            if agg == 'composite':
                whole, part = src, dst
            elif agg == 'shared':
                whole, part = src, dst
        if whole and part:
            target = compositions if agg == 'composite' else aggregations
            target.append({'type':'composition' if agg=='composite' else 'aggregation',
                           'whole':whole,'part':part,'multiplicity':dstMult})
        else:
            associations.append({'type':'association','source':src,'target':dst,
                                 'multiplicity1':srcMult,'multiplicity2':dstMult})
    return associations, compositions, aggregations
...
try:
    ...
    # Descomponer los valores retornados por extract_associations
    associations, compositions, aggregations = extract_associations(root)

    # Concatenar las listas correctamente
    relationships = generalizations + directed_associations + associations + compositions + aggregations
